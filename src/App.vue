  ...
  <v-app dark class="head">
    <v-container fluid class="app">
      <v-flex md3 lg2 class="zoom hidden-sm-and-down pa-4" style="position:fixed; z-index: 100; left:0; top:0;">
        <v-layout column>
          <h1 style="font-family: 'Indie Flower', sans-serif;">
      ...
      <v-flex md3 lg2 class="zoom pa-4" style="position:fixed; z-index: 100; right:0; top:0;">
        <v-tooltip bottom color="amber" v-if="!user"><span class="text--text">Log in with Google</span><v-btn class="ma-2" large slot="activator" color="amber" fab flat id="b1" @click="GoogleSignIn" :loading="loading"><v-icon large>fab fa-google</v-icon></v-btn></v-tooltip>
        <v-tooltip bottom color="amber" v-if="user"><span class="text--text text-xs-center">Logout</span>
      ...
      <v-layout row wrap>
        ...
        <v-flex xs12 sm12 md6 offset-md3 lg8 offset-lg2 class="about" id="about">
...
.head::before{
  ...
  height: 100%;
  position: absolute;
  background-image: url('https://upload.wikimedia.org/wikipedia/commons/9/90/Touched_by_His_Noodly_Appendage_HD.jpg');
